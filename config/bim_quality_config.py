...
class BIMQualityConfig:
    ...
    # Sigmoid 함수 파라미터
    # 웹서치 근거: 실제 BIM 프로젝트에서 설계오류 30% 감소, RFI 25% 감소
    # → 실제 탐지율은 60-75% 수준 (100% 불가능)
    SIGMOID_K = 6       # 기울기 (10→6: 더 완만한 곡선, 품질 차이 반영)
    SIGMOID_X0 = 0.6    # 변곡점 (0.5→0.6: 높은 BIM 품질 요구)

    # 최대 탐지 확률 (현실적 수준)
    # 근거: 해외 연구(2024-2025) - 설계오류 감소 30%, 재작업 감소 30%
    # 한국 사례(현대건설) - "사전 오류 발견으로 소모적 업무 확연히 감소"
    # → BIM 우수 품질에서도 70-75% 탐지가 현실적
    MAX_DETECTION_PROB = 0.75  # 98%→75%: 인적오류, BIM 입력오류, 예측불가 상황 반영
